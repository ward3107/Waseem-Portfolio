  ...
  return (
    <section id="process" className="bg-slate-950 text-white relative">
      ...
      <div className="hidden lg:flex">
        ...
        <div className="w-1/2 h-screen sticky top-0 flex items-center justify-center bg-slate-900 border-r border-slate-800 overflow-hidden relative">
           ...
           <div className="relative z-20 w-[400px] h-[400px] flex items-center justify-center">
              {steps.map((step, index) => (
                <motion.div
                    ...
                    className="absolute inset-0 flex flex-col items-center justify-center p-8 rounded-full border-4 border-slate-800 bg-slate-900 shadow-2xl"
                    style={{ 
        ...
        <div className="w-1/2">
           {steps.map((step, index) => (
             <div 
                key={step.id} 
                className={`process-step-text min-h-screen flex items-center px-16 py-20 border-b border-slate-900/50 ${activeStep === index ? 'opacity-100' : 'opacity-30'} transition-opacity duration-500`}
             >
                <div>
                   ...
                   <h3 className="text-4xl font-heading font-bold mb-6 text-white leading-tight">
                     {step.title}
                   </h3>
                   ...
                   <div className="flex items-center gap-2 text-sm font-bold text-white group cursor-pointer">
                      <div className={`w-8 h-[2px] ${step.bg.replace('bg-', 'bg-') === step.bg ? step.bg.replace('/10','/100').replace('text-','bg-') : 'bg-white'} transition-all group-hover:w-16`}></div>
      ...
      <div className="lg:hidden py-20 px-6 sm:px-10">
         <div className="text-center mb-16">
            ...
            <p className="text-slate-400">{t('process_mobile_desc')}</p>
         </div>
         ...
         <div className="relative space-y-12">
            ...
            {steps.map((step, index) => (
                ...
                >
                    {/* Icon Bubble */}
                    <div className={`relative z-10 flex-shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center border border-slate-800 bg-slate-900 shadow-xl ${step.color}`}>
                        <step.icon size={28} />
                        {/* Connector dot */}
                        <div className={`absolute -bottom-12 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-slate-800 border-2 border-slate-950 ${index === steps.length - 1 ? 'hidden' : ''}`}></div>
                    </div>
                    ...
                    <div className="pt-2 pb-8">
                        ...
                        <div className="text-slate-400 text-sm leading-relaxed border-l-2 border-slate-800 pl-4">
                            {step.desc}
                        </div>
                    </div>
                </motion.div>
